...
namespace Pythia8 {
    ...
    bool BeamRemnants::setKinematics(Event& event) {
        ...
        event.TuningmHatDampCollection.clear();
        event.TuningGaussPtVector.clear();
        event.TuningScaleCollection.clear();
        ...
        vector<double> StoreScale(nMaxBeam);
        ...
        for (int iSys = 0; iSys < nSys; ++iSys) {
            ...
            if (doPrimordialKT) {
                ...
                double yDamp = pow((event[iInA].e() + event[iInB].e()) / mHat,
                        reducedKTatHighY);
                //std::cout<<"ReducedKTHigh "<<reducedKTatHighY<<std::endl;
                ...
                if ((beam.isHadron() || beam.isGamma()) && doPrimordialKT) {
                    ...
                    for (int iPar = 0; iPar < nPar; ++iPar) {
                        if (beam[iPar].isFromBeam()) {
                            ...
                            if ((int) iPar < nSys) {
                                event.TuningGaussPtVector.push_back(sqrt(gauss2.first * gauss2.first + gauss2.second * gauss2.second));
                                event.TuningmHatDampCollection.push_back(kTcomp[iPar]);
                                event.TuningScaleCollection.push_back(StoreScale[iPar]);
                            }
                            beam[iPar].px(px);
                            beam[iPar].py(py);
                            pxSum += px;
